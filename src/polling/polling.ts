...
class Polling<T = unknown> {
  ...
  public polling(runner: PollingRunner<T>): PollingCancel {
    ...
    if (!pollingOn || !interval) throw Error('Must have a pollingOn function when polling');
    ...
    return () => intervalId !== null && intervalId !== undefined && clearInterval(intervalId)
  }

  private _run(runner: PollingRunner<T>): Promise<T | undefined> {
    const { pollingOn } = this._init;
    if (!pollingOn) throw new Error('Must have a pollingOn function when polling')
